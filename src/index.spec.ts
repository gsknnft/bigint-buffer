...
let bigintToBuf: Awaited<typeof import('./index.js')>['bigintToBuf'];
let bufToBigint: Awaited<typeof import('./index.js')>['bufToBigint'];
let bigintToHex: Awaited<typeof import('./index.js')>['bigintToHex'];
let hexToBigint: Awaited<typeof import('./index.js')>['hexToBigint'];
let bigintToText: Awaited<typeof import('./index.js')>['bigintToText'];
let textToBigint: Awaited<typeof import('./index.js')>['textToBigint'];
let bigintToBase64: Awaited<typeof import('./index.js')>['bigintToBase64'];
let base64ToBigint: Awaited<typeof import('./index.js')>['base64ToBigint'];
...
describe('Try bigint conversion (little endian)', () => {
  it('0 should equal 0n', () => {
    expect(toBufferLE(BigInt('0'), 8)).toEqual(Buffer.from([
      0, 0, 0, 0, 0, 0, 0, 0
    ]));
  });

  it('1 should equal 1n', async () => {
    expect(toBufferLE(BigInt('1'), 8)).toEqual(Buffer.from([
      1, 0, 0, 0, 0, 0, 0, 0
    ]));
  });

  it('1 should equal 1n (32 byte)', async () => {
    expect(toBufferLE(BigInt('1'), 32))
        .toEqual(Buffer.from(
  ...
  it('0xdead should equal 0xdeadn (6 byte)', async () => {
    expect(toBufferLE(BigInt('0xdead'), 6)).toEqual(Buffer.from([
      0xad, 0xde, 0, 0, 0, 0
    ]));
  });

  it('0xdeadbeef should equal 0xdeadbeef0000000000n (9 byte)', async () => {
    expect(toBufferLE(BigInt('0xdeadbeef'), 9)).toEqual(Buffer.from([
  ...
  it('0xbadc0ffee0ddf00d should equal 0xbadc0ffee0ddf00dn (8 byte)',
     async () => {
       expect(toBufferLE(BigInt('0xbadc0ffee0ddf00d'), 8)).toEqual(Buffer.from([
         0x0d, 0xf0, 0xdd, 0xe0, 0xfe, 0x0f, 0xdc, 0xba
       ]));
     });
  ...
  it('0xbadc0ffee0ddf00ddeadbeef should equal 0xbadc0ffee0ddf00ddeadbeefn',
     async () => {
       expect(toBufferLE(BigInt('0xbadc0ffee0ddf00ddeadbeef'), 12))
           .toEqual(Buffer.from([
             0xef, 0xbe, 0xad, 0xde, 0x0d, 0xf0, 0xdd, 0xe0, 0xfe, 0x0f, 0xdc,
             0xba
           ]));
  ...
  it('long value should equal long val', async () => {
    expect(
        toBufferLE(
            BigInt('0xbadc0ffee0ddf00ddeadbeefbadc0ffee0ddf00ddeadbeef'), 24))
        .toEqual(Buffer.from([
          0xef, 0xbe, 0xad, 0xde, 0x0d, 0xf0, 0xdd, 0xe0,
          0xfe, 0x0f, 0xdc, 0xba, 0xef, 0xbe, 0xad, 0xde,
          0x0d, 0xf0, 0xdd, 0xe0, 0xfe, 0x0f, 0xdc, 0xba
...
describe('Try bigint conversion (big endian)', () => {
  it('0 should equal 0n', () => {
    expect(toBufferBE(BigInt('0'), 8)).toEqual(Buffer.from([
      0, 0, 0, 0, 0, 0, 0, 0
    ]));
  });

  it('1 should equal 1n', async () => {
    expect(toBufferBE(BigInt('1'), 8)).toEqual(Buffer.from([
      0, 0, 0, 0, 0, 0, 0, 1
    ]));
  });

  it('1 should equal 1n (32 byte)', async () => {
    expect(toBufferBE(BigInt('1'), 32))
        .toEqual(Buffer.from(
  ...
  it('0xdead should equal 0xdeadn (6 byte)', async () => {
    expect(toBufferBE(BigInt('0xdead'), 6)).toEqual(Buffer.from([
      0, 0, 0, 0, 0xde, 0xad
    ]));
  });

  it('0xdeadbeef should equal 0xdeadbeef0000000000n (9 byte)', async () => {
    expect(toBufferBE(BigInt('0xdeadbeef'), 9)).toEqual(Buffer.from([
  ...
  it('0xbadc0ffee0ddf00d should equal 0xbadc0ffee0ddf00dn (8 byte)',
     async () => {
       expect(toBufferBE(BigInt('0xbadc0ffee0ddf00d'), 8)).toEqual(Buffer.from([
  ...
  it('0xbadc0ffee0ddf00ddeadbeef should equal 0xbadc0ffee0ddf00ddeadbeefn',
     async () => {
       expect(toBufferBE(BigInt('0xbadc0ffee0ddf00ddeadbeef'), 12))
           .toEqual(Buffer.from([
