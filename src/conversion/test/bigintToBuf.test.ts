    ...
    describe(`bufToBigint(bigintToBuf(${input}))`, () => {
      if (input < 0) {
        it('should throw RangeError', () => {
          expect(() => bc.bufToBigint(bc.bigintToBuf(input))).to.throw(RangeError);
        });
        it('should throw RangeError', () => {
          expect(() => bc.bufToBigint(bc.bigintToBuf(input, true))).to.throw(RangeError);
        });
        it('should throw RangeError', () => {
          expect(() => bc.bufToBigint(bc.bigintToBuf(input, false))).to.throw(RangeError);
        });
      } else {
